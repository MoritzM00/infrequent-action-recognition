...
import infreqact.data.transforms.functional
...
class UniformTemporalSubsample(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return infreqact.data.transforms.functional.uniform_temporal_subsample(
...
class UniformTemporalSubsampleRepeated(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return infreqact.data.transforms.functional.uniform_temporal_subsample_repeated(
...
class ShortSideScale(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return infreqact.data.transforms.functional.short_side_scale(
...
class UniformCropVideo(torch.nn.Module):
    ...
    def __call__(self, x: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        Args:
            x (Dict[str, torch.Tensor]): video clip dict.
        """
        x[self._video_key] = infreqact.data.transforms.functional.uniform_crop(
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return infreqact.data.transforms.functional.random_resized_crop(
            x,
            self._target_height,
            self._target_width,
            self._scale,
...
class OpSampler(torch.nn.Module):
    ...
    def __init__(
        ...
        transforms_prob: list[float] | None = None,
        num_sample_op: int = 1,
...
class Div255(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torchvision.transforms.Lambda(infreqact.data.transforms.functional.div_255)(x)
