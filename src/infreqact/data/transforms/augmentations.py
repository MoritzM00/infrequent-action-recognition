...
def _rotate(video: torch.Tensor, factor: float, **kwargs) -> torch.Tensor:
    ...
    return torchvision.transforms.rotate(
...
class AugmentTransform:
    def __init__(
        ...
        magnitude: int = 10,
        prob: float = 0.5,
        name_to_transform_func: dict[str, Callable] | None = None,
        level_to_arg: dict[str, Callable] | None = None,
        transform_max_paras: dict[str, tuple] | None = None,
        transform_hparas: dict[str, Any] | None = None,
        sampling_type: str = "gaussian",
        sampling_hparas: dict[str, Any] | None = None,
    ) -> None:
        """
...
def _apply_grid_transform(
    img: Tensor, grid: Tensor, mode: str, fill: int | float | list[float] | None
) -> Tensor:
