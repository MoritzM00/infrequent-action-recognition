...
class SpectralTransform(torch.nn.Module):
    ...
    def forward(self, x):
        if type(x) is dict:
            frames = x[
                "pixel_values"
            ]  # frames could also be a batch, e.g,  torch.Size([4, 16, 3, 224, 224])
            if self.fp16:
    ...
    def spectral_apply_video(self, frames, fn):
        ...
        if self.denormalize_vals is not None:
            ...
            mean = torch.tensor(
                self.denormalize_vals["mean"], device=current_device, dtype=frames.dtype
            )
            std = torch.tensor(
                self.denormalize_vals["std"], device=current_device, dtype=frames.dtype
            )
        ...
        if self.normalize_vals is not None:
            # Create tensors on the same device as frames
            mean = torch.tensor(
                self.normalize_vals["mean"], device=current_device, dtype=frames.dtype
            )
            std = torch.tensor(
                self.normalize_vals["std"], device=current_device, dtype=frames.dtype
            )
