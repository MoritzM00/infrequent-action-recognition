...
class TubeMaskingGenerator:
    ...
    def __repr__(self):
        repr_str = f"Mask: total patches {self.total_patches}, mask patches {self.total_masks}"
        return repr_str
...
class TubeMasker:
    ...
    def __call__(self, x):
        ...
        device = (
            input_tensor.device if isinstance(input_tensor, torch.Tensor) else torch.device("cpu")
        )

        # Check if we have a batch dimension
        has_batch = (
            len(input_tensor.shape) > 4
        )  # For videos, shape would be [t,c,h,w] or [b,t,c,h,w]
