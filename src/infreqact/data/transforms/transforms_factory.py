...
from infreqact.data.transforms.augmix import AugMix
from infreqact.data.transforms.rand_augment import RandAugment
...
def _get_augmentation(aug_type: str, aug_paras: dict[str, Any] | None = None) -> list[Callable]:
    """
...
def create_video_transform(
    ...
    remove_key: list[str] | None = None,
    num_samples: int | None = 8,
    ...
    aug_paras: dict[str, Any] | None = None,
    random_resized_crop_paras: dict[str, Any] | None = None,
) -> (
    ...
    if random_resized_crop_paras is not None:
        ...
        if "aspect_ratio" not in random_resized_crop_paras:
            random_resized_crop_paras["aspect_ratio"] = _RANDOM_RESIZED_CROP_DEFAULT_PARAS[
                "aspect_ratio"
            ]
...
class ToTensorVideo:
    ...
    def __call__(self, clip):
        ...
        clip = clip.permute(3, 0, 1, 2)  # (T, H, W, C) -> (C, T, H, W)
        return clip
